...
#define SEM_NAME_ALICE_TO_BOB "/sem_alice_to_bob"
#define SEM_NAME_BOB_TO_ALICE "/sem_bob_to_alice"
...
void send(const Message *message, void *ptr)
{
    memcpy(ptr, message, message->size);
    sem_post(sem_open(SEM_NAME_ALICE_TO_BOB, 0));
}

const Message *recv(void *ptr)
{
    sem_wait(sem_open(SEM_NAME_BOB_TO_ALICE, 0));
    static Message *m = (Message *)malloc(MESSAGE_SIZES[4]);
    memcpy(m, ptr, sizeof(Message));
    memcpy(m->payload, ((char *)ptr) + sizeof(Message), m->payload_size());
    return m;
}
